...
uint8_t mos6502 :: read(const uint16_t& addr) const {
    return bus->read(addr, false);
}

void mos6502 :: write(const uint16_t& addr, const uint8_t& data) {
     bus->write(addr, data);
}
...
void mos6502 :: clock() {
    if(clock == 0) {
        opcode = read(pc);
        pc++; // Increment PC to point to the next byte in memory
...
void mos6502 :: setFlag(FLAGS flag, const bool& v) {
    if(v) status |= flag;
    else status &= ~flag; 
}

uint8_t mos6502 :: getFlag(const FLAGS& flag) const {
